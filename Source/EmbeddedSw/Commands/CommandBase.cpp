...
void CommandBase::commandInitiatorInit(commandInitiatorId_t commandInitiatorId, commandSequenceNumber_t commandSequenceNumber)
{
	if (commandInitiatorId == commandInitiatiorIdPythonUtilities)
	{
		...
		// Preserve the sequence number generated by python (used by python to make sure returning correct command)
	...
	else
	{
		...
		if (commandInitiatorId == commandInitiatorIdDefault)
		{
			m_commandInitiatorId = commandInitiatorIdDefault;
		}
		else
		{
			// use commandInitiatorIdOutOfRangeInitiatorId rather than throw an error as
			// part of constructor and/or system initialization
			m_commandInitiatorId = commandInitiatorIdOutOfRangeInitiatorId;
		}
	}
}
